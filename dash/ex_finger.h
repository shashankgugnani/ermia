...
struct Directory {
  ...
  static void New(Directory **dir, size_t capacity, size_t _version){
    // TODO(tzwang): use ERMIA allocator
    int ret = posix_memalign((void **)dir, kCacheLineSize, sizeof(Directory<T>) + sizeof(table_p) * capacity);
    ALWAYS_ASSERT(ret == 0);
    memset(*dir, 0, sizeof(Directory<T>) + sizeof(table_p) * capacity);
...
struct Table {
  static void New(Table<T> **tbl, size_t depth, Table<T>* pp) {
    // TODO(tzwang): use ERMIA allocator
    int ret = posix_memalign((void **)tbl, kCacheLineSize, sizeof(Table<T>));
    ALWAYS_ASSERT(ret == 0);
